...
const CONFIG = {
  SHEET_ID: '1MDlUN3Qs9NRLgXZyNG8YxwXEpHwc4vF3VH2vXbd6Gzg', // ✅ NUOVO SHEET
  ...
  // Mappatura colonne per MUD (secondo screenshot)
  COLUMNS_MUD: {
    TIMESTAMP: 1,        // A - Timestamp
    UTENTE: 2,          // B - Utente
    MUD: 3,             // C - MUD
    LUOGO: 4,           // D - Luogo
    DATA_INIZIO: 5,     // E - Data inizio
    DATA_FINE: 6,       // F - Data fine
    DESCRIZIONE: 7,     // G - Descrizione
    MATERIALI: 8,       // H - Materiali
    FIRMA_COMMITTENTE: 9, // I - Firma Committente
    BUONO_LAVORO: 10    // J - Buono di lavoro
  },
  // Mappatura colonne per Interventi (secondo screenshot)
  COLUMNS_INTERVENTI: {
    TIMESTAMP: 1,        // A - Timestamp
    UTENTE: 2,          // B - Utente
    BUONO_LAVORO: 3,    // C - Buono di lavoro
    LUOGO: 4,           // D - Luogo
    DATA_INIZIO: 5,     // E - Data inizio
    DATA_FINE: 6,       // F - Data fine
    DESCRIZIONE: 7,     // G - Descrizione
...
function configuraGoogleSheetDueFogli(nuovoSheetId) {
  ...
  try {
    ...
    // Verifica/Crea foglio Interventi
    let interventiSheet;
    try {
      interventiSheet = ss.getSheetByName(CONFIG.SHEET_NAMES.INTERVENTI);
      if (!interventiSheet) {
        interventiSheet = ss.insertSheet(CONFIG.SHEET_NAMES.INTERVENTI);
        console.log('📋 Foglio Interventi creato');
      }
    } catch (error) {
      interventiSheet = ss.insertSheet(CONFIG.SHEET_NAMES.INTERVENTI);
      console.log('📋 Foglio Interventi creato (fallback)');
    }
    // Intestazioni per foglio MUD
    const headerRowMUD = [
      'Timestamp',
      'Utente',
      'MUD',
    ...
    // Intestazioni per foglio Interventi
    const headerRowInterventi = [
      'Timestamp',
      'Utente',
      'Buono di lavoro',
      'Luogo',
    ...
    if (interventiSheet.getDataRange().getNumRows() === 0) {
      interventiSheet.getRange(1, 1, 1, headerRowInterventi.length).setValues([headerRowInterventi]);
      console.log('✅ Intestazioni Interventi aggiunte');
    }
    return {
      success: true,
      ...
      interventiSheetName: interventiSheet.getName(),
...
function saveDataToCorrectSheet(params) {
  try {
    ...
    const sheetName = isMUD ? CONFIG.SHEET_NAMES.MUD : CONFIG.SHEET_NAMES.INTERVENTI;
    const columns = isMUD ? CONFIG.COLUMNS_MUD : CONFIG.COLUMNS_INTERVENTI;
    ...
    let buonoLavoro;
    if (isMUD) {
      // MUD: genera buono lavoro come sempre
      ...
      if (existingRowIndex !== -1) {
        ...
        for (let col = 1; col <= totalColumns; col++) {
          if (col !== columns.FIRMA_COMMITTENTE) {
      ...
      // Interventi: buono lavoro in colonna 3
      if (existingRowIndex !== -1) {
        buonoLavoro = existingData[existingRowIndex][columns.BUONO_LAVORO - 1];
        if (!buonoLavoro || buonoLavoro === 'N/A') {
          buonoLavoro = generaBuonoLavoro(data.user, sheet);
    ...
    return {
      ...
      phase: 'DATA_SAVED',
      sheetType: isMUD ? 'MUD' : 'Interventi',
      sheetName: sheetName,
...
function uploadClientSignature(params) {
  try {
    ...
    for (let i = 1; i < values.length; i++) { // Skip header
      ...
      if (isMUD) {
        // MUD: cerca per MUD
        const existingMud = values[i][columns.MUD - 1];
        if (existingMud === mudValue || existingMud === identificativo) {
          match = true;
        }
      } else {
        // Interventi: cerca per Buono di lavoro
        const existingBuono = values[i][columns.BUONO_LAVORO - 1];
        if (existingBuono === identificativo) {
          match = true;
        }
      }
      if (match) {
        buonoLavoro = isMUD ? values[i][columns.BUONO_LAVORO - 1] : values[i][columns.BUONO_LAVORO - 1];
        if (isMUD) {
...
function testScriptDueFogli() {
  ...
  console.log('  Colonne Interventi:', Object.keys(CONFIG.COLUMNS_INTERVENTI).length);
  ...
  // Test dati Interventi
  const testDataInterventi = {
    user: 'admin',
    tipoIntervento: 'interventi',
    luogo: 'Roma - Test Interventi',
    dataInizio: '2025-11-15',
    dataFine: '2025-11-15',
    descrizione: 'Test intervento Interventi',
    materiali: 'Materiali test Interventi'
  };

  console.log('📤 Test invio MUD:', testDataMUD);
  console.log('📤 Test invio Interventi:', testDataInterventi);
  ...
  try {
    ...
    // Test Interventi
    const paramsInterventi = {
      parameter: {
        callback: 'testCallback',
        action: 'save',
        data: encodeURIComponent(JSON.stringify(testDataInterventi))
      }
    };

    const resultInterventi = handleJsonpRequest(paramsInterventi);
    console.log('✅ Test Interventi risultato:', resultInterventi.getContent());
